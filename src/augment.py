...
def grab_random_audio_excerpts(spects, labels, batchsize, sample_rate, frame_len, fps, frames):
    """
    Exctracts random excerpts of `frames` frames from the spectrograms in
    `spects` paired with the label from `labels` associated with the central
    frame. Yields `batchsize` excerpts at a time. Draws without replacement
    ...
    hop = sample_rate//fps
    # buffer to write each batch into
    batch_spects = np.empty((batchsize, (frames-1)*hop+frame_len),
                            dtype=spects[0].dtype)
    batch_labels = np.empty((batchsize,) + labels[0].shape[1:],
                            dtype=labels[0].dtype)
    #for spect_idx, spect in enumerate(spects):
    #    print((len(spect) - frame_len)//hop  + 1-frames+1)
    #    print(len(np.arange(0,(len(spect)-frame_len-(frames-2)*hop)//hop *hop , hop,
    #                                         dtype=np.int)))
    #    print((len(spect)-frame_len-(frames-2)*hop)/hop)
    # array of all possible (spect_idx, frame_idx) combinations
    indices = np.vstack(np.vstack((np.ones((len(spect) - frame_len)//hop  + 1-frames+1,
                                           dtype=np.int) * spect_idx,
                                   np.arange(0,(len(spect)-frame_len-(frames-2)*hop)//hop *hop , hop,
                                             dtype=np.int))).T
                        for spect_idx, spect in enumerate(spects)
                        if len(spect) >= frames)
    # infinite loop
    while True:
        ...
        b = 0
        for spect_idx, frame_idx in indices:
            batch_spects[b] = spects[spect_idx][frame_idx:frame_idx + (frames-1)*hop + frame_len]
            batch_labels[b] = labels[spect_idx][frame_idx + hop*frames//2]
            b += 1
            if b == batchsize:
                # copy the buffers to prevent changing returned data (not a
                # problem if it is consumed right away, but if it is collected)
...
def grab_excerpts(spects, labels, batchsize, frames):
    """
    Exctracts random excerpts of `frames` frames from the spectrograms in
    `spects` paired with the label from `labels` associated with the central
    frame. Yields `batchsize` excerpts at a time. Draws without replacement
