...
def read_ffmpeg(infile, sample_rate, cmd='ffmpeg'):
    ...
    #call = [cmd,"-v", "quiet", "-i", infile,"-f", "f32le","-af","pan=mono|c0=0.5*c0+0.5*c1", "pipe:1"]
    samples = subprocess.check_output(call)
    return np.frombuffer(samples, dtype=np.float32)

def read_librosa(infile, sample_rate):
    """
    Decodes audio file using librosa core load, resampled to a given sample
    rate using kaiser best, downmixed to mono and converted to float32. Terturn numpy array.
    """
    y, sr = librosa.core.load(infile, sr=sample_rate, mono=True,
            dtype=np.float32)
    return y
...
def extract_spect(filename, sample_rate=22050, frame_len=1024, fps=70):
    ...
    try:
        ...
        samples = read_librosa(filename, sample_rate)
